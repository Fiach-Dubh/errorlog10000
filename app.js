(() => {
  ...
  function renderCards(events) {
    ...
    const html = shownEvents.map((event) => {
      ...
      return divider + `<li class="card" data-id="${escapeAttr(event.id)}" data-category="${escapeAttr(event.category)}" data-visual="${escapeAttr(visualClass)}" data-email-contact="${escapeAttr(event.email_contact || "")}" tabindex="0"><div class="card-meta"><span>${escapeHtml(formatTimelineDate(event))}</span><span class="badge">${escapeHtml(event.category)}</span><span>${escapeHtml(event.email_contact || metaExtra)}</span></div><h3>${escapeHtml(event.title)}</h3><blockquote class="quote-preview">${cardExcerptHtml(event)}</blockquote>${cardActions}</li>`;
    }).join("");
  ...
  function openEvent(id) {
    ...
    dialogBody.innerHTML = `<article class="dialog-content"><p class="eyebrow">Entry ${event.seq}</p><h2>${escapeHtml(event.title)}</h2><div class="dialog-meta"><span>${escapeHtml(formatTimelineDate(event))}</span><span>${escapeHtml(event.category)}</span>${contactMeta}<span>${escapeHtml(event.source)}</span><span>${escapeHtml(event.page ? String(event.page) : "")}</span></div><div class="quote-full">${safeRichHtml(event, "quote_html", "quote")}</div><div class="dialog-actions"><button class="dialog-copy" type="button" data-copy="quote">Copy quote</button>${links}</div></article>`;
    dialog.showModal();
  }
