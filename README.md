# Latest June 5 Bohm transactions, support popup, and map-drag update

- Added three Nicholas Bohm/Satoshi Bitcoin transaction cards to the **Nicholas Bohm Emails** lane, with UTC timestamps, block/address details, and source links.
- Added the **Support The Site** image popup in the lower-right corner after **2 minutes 10 seconds**, with an **X** close button.
- Made the Timechain map’s left-click-and-hold grab interaction functional so users can drag the map horizontally.

# Latest June 3 lane cleanup and spacing update

- Moved 9 private Nicholas Bohm/Satoshi email entries from **SourceForge / Bitcoin list** into **Nicholas Bohm Emails**.
- Adjusted the Timechain map vertical spacing: the central Timechain is now lower/centered, with clearer separation from **Other** above and **P2P Foundation** below.
- Bolded Entry **3118** and Entry **3127** as Satoshi-authored Nicholas Bohm email cards.


## Latest June 2 performance and timeline-controls update

- Added small left/right fine-scroll arrow buttons under the Timechain map scrollbar for one-click horizontal nudges.
- Reduced initial page load pressure by starting the year range at **2008** instead of the full 2008–2014 span. Drag the range control right to reveal later years.
- Changed chronological cards to load in chunks of **10** with a **Load 10 more cards** button.
- Removed the long verified spreadsheet filename from visible card/dialog metadata and from bundled dataset metadata.
- Updated the Wei Dai reply card to **Saturday, August 23, 2008 12:21 PM UTC** so its timeline placement matches the supplied UTC timestamp.

## Latest rich-text formatting update

- Preserved bold and italic rich-text formatting from column B of `the verified chronological timeline spreadsheet`.
- Timeline card previews now render the formatted excerpt, and the full entry dialog renders the formatted full body.
- Kept rows 168/169 combined into a single card while preserving the formatting from both cells.

## Latest verified chronological dataset update

- Replaced the old **Kicking the Hornet's Nest**-derived timeline entries for **Email**, **Cryptography Mailing List**, **SourceForge / Bitcoin list**, **bitcoin.org/smf Forums**, and **P2P Foundation** with the verified spreadsheet dataset `the verified chronological timeline spreadsheet`.
- Preserved the existing **Patoshi Blocks**, **Theymos Missing Quotes**, **Whitepaper**, **Code Commits**, and **Other** lanes.
- Added a new **Nicholas Bohm Emails** lane; after the June 3 cleanup it contains **62** email entries.
- Combined spreadsheet rows **168/169** into one timeline card.
- Added support for multiple source links per entry, using the links from column D of the spreadsheet.
- Added contact-specific email colors for Hal Finney, Adam Back, Wei Dai, Mike Hearn, Dustin Trammell, Martti Malmi, Nicholas Bohm, Gavin Andresen, and Martien van Steenbergen.
